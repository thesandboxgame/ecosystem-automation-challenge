...
describe('Internationalization', () => {
    ...
    it('should load the 日本 language', () => {
      ...
      cy.contains('あなたの内なるバニーをチャネリングしてください。ラビターズと肩をこすります。プレイボーイのパーティーピープルと一緒にパーティーしよう！'); // The text is not properly displayed when translated to JP. This can be fixed very quickly if this is not the desired behaivor.
    });
    ...
    it('should load the Türk language', () => {
      ...
      cy.contains('İçinizdeki Tavşanı kanalize edin. Rabbitars ile omuz omuza. Playboy Parti İnsanları ile Parti !'); // The text is not properly displayed when translated to JP. This can be fixed very quickly if this is not the desired behaivor.
    });
  });